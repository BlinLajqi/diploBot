...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (target_channel) {
      await target_channel.bulkDelete(amount).then(async (messages) => {
        Response.setDescription(
          `🧹 Deleted ${messages.size} from ${target_channel}.`
        );
      ...
      await channel.bulkDelete(amount).then(async (messages) => {
